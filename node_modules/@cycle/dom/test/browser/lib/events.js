...
describe('DOMSource.events()', function () {
        ...
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.parent', [
                    lib_1.h3('.myelementclass', 'Foobar'),
                ])),
            };
        }
        var _a = run_1.setup(app, {
        ...
        if (!thisBrowserBubblesFragmentEvents) {
            ...
            sources.DOM.select('.clickable').events('click', { useCapture: false }).addListener({
                next: function (ev) {
                    ...
                    var renderTarget = top.parentNode;
                    var frag = renderTarget.parentNode;
    ...
    it('should have currentTarget or ownerTarget pointed to the selected parent', function (done) {
        function app(sources) {
            return {
                DOM: xstream_1.default.of(lib_1.div('.top', [
                    lib_1.h2('.parent', [
                        lib_1.span('.child', 'Hello world'),
                    ]),
                ])),
            };
        }
        ...
        sources.DOM.select('.parent').events('click').addListener({
            next: function (ev) {
                ...
                var currentTargetIsParentH2 = currentTarget.tagName === 'H2' && currentTarget.className === 'parent';
    ...
    it('should not simulate bubbling for non-bubbling events', function (done) {
        ...
        sources.DOM.select('.form').events('reset').compose(delay_1.default(200)).addListener({
            next: function (ev) {
                assert.strictEqual(ev.type, 'reset');
                var target = ev.target;
                assert.strictEqual(target.tagName, 'FORM');
